...
int kSeries :: sendRequest(int reqType, int respSize, int respInd)
{
	...
	while(this->_Serial->available() < respSize)	//loop through until we have are 7-byte response
	{ 
	...
	if(this->_Serial->available() == respSize)		//if we have our 7-byte response get value
